        ...
        if (paths.length === 0) {
            ...
            paths.forEach(function (path) {
                if (!paths.find(function (item) { return RegExp("^".concat(path, ".")).test(item); })) {
                    console.log('the data:', { data: _this.data.value, path: path, param1: param1 });
                    _this.data.value[''] ? output.set(param1['']) : (path ? output.set(path, input.get(path)) : output.set(input.get()));
                }
